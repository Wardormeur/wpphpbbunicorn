...
function gen_sort_selects(&$limit_days, &$sort_by_text, &$sort_days, &$sort_key, &$sort_dir, &$s_limit_days, &$s_sort_key, &$s_sort_dir, &$u_sort_param, $def_st = false, $def_sk = false, $def_sd = false)
{
    ...
    $sorts = array('st' => array('key' => 'sort_days', 'default' => $def_st, 'options' => $limit_days, 'output' => &$s_limit_days), 'sk' => array('key' => 'sort_key', 'default' => $def_sk, 'options' => $sort_by_text, 'output' => &$s_sort_key), 'sd' => array('key' => 'sort_dir', 'default' => $def_sd, 'options' => $sort_dir_text, 'output' => &$s_sort_dir));
    $u_sort_param = '';
...
function make_jumpbox($action, $forum_id = false, $select_all = false, $acl_list = false, $force_display = false)
{
    ...
    while ($row = $db->sql_fetchrow($result)) {
        ...
        if (!$display_jumpbox) {
            $template->assign_block_vars('jumpbox_forums', array('FORUM_ID' => $select_all ? 0 : -1, 'FORUM_NAME' => $select_all ? $user->lang['ALL_FORUMS'] : $user->lang['SELECT_FORUM'], 'S_FORUM_COUNT' => $iteration, 'LINK' => $phpbb_path_helper->append_url_params($action, array('f' => $forum_id))));
            $iteration++;
            $display_jumpbox = true;
        }
        $template->assign_block_vars('jumpbox_forums', array('FORUM_ID' => $row['forum_id'], 'FORUM_NAME' => $row['forum_name'], 'SELECTED' => $row['forum_id'] == $forum_id ? ' selected="selected"' : '', 'S_FORUM_COUNT' => $iteration, 'S_IS_CAT' => $row['forum_type'] == FORUM_CAT ? true : false, 'S_IS_LINK' => $row['forum_type'] == FORUM_LINK ? true : false, 'S_IS_POST' => $row['forum_type'] == FORUM_POST ? true : false, 'LINK' => $phpbb_path_helper->append_url_params($action, array('f' => $row['forum_id']))));
        for ($i = 0; $i < $padding; $i++) {
    ...
    $template->assign_vars(array('S_DISPLAY_JUMPBOX' => $display_jumpbox, 'S_JUMPBOX_ACTION' => $action, 'HIDDEN_FIELDS_FOR_JUMPBOX' => build_hidden_fields($url_parts['params'])));
    return;
}
/**
...
function phpbb_add_quickmod_option($url, $option, $lang_string)
{
    ...
    $template->assign_block_vars('quickmod', array('VALUE' => $option, 'TITLE' => $lang_string, 'LINK' => $phpbb_path_helper->append_url_params($url, array('action' => $option))));
}
/**
